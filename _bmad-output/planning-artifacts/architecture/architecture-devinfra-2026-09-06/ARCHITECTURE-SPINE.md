...
- **Rule:** Named volumes keep their current identifiers (`postgres-data`, `keycloak-data`, `minio-data`, …) permanently. Renaming a Module never renames its volume — **the `object-storage` Module keeps the volume `minio-data`**, and the `<service-name>-data` convention yields to this rule wherever the two disagree. Freezing wins; tidiness does not. A Module's `volumes:` and `networks:` top-level stanzas contain **the identifier and nothing else** — no `driver`, no `driver_opts`, no `attachable`. All such keys live in the Core `compose.yaml`. CI diffs every Module's declaration against Core and fails on any extra key.
